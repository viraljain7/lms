...
const CourseDetails = () => {
  ...
  const enrollCourse = async () => {
    try {
      ...
      if (data.success) {
        const { razorpaySession } = data;
        console.log(data)
console.log(data)
        const options = {
          key: razorpaySession.key,
          amount: razorpaySession.amount/100,
          currency: razorpaySession.currency,
          name: "Course Purchase",
          description: "Course Payment",
          order_id: razorpaySession.orderId,

          handler: function (response) {
            // payment success (UI only)
            window.location.href = razorpaySession.redirectUrl;
          },
        ...
        rzp.open(); // 🔥 THIS LINE OPENS PAYMENT PAGE
      } else {
        toast.error(data.message);
      }
    } catch (error) {
