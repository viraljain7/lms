...
const CourseDetails = () => {
  ...
  // PayU Enrollment
const enrollCoursePayU = async () => {
  if (!courseData) return toast.error("Course data not loaded.");
  ...
  try {
    ...
    const { data } = await axios.post(
      ...
      { courseId: courseData._id, gateway: "PAYU" },
      { headers: { Authorization: `Bearer ${token}` } }
    );

    if (!data.success) return toast.error(data.message);

    const payuSession = data.payuSession;

    // =================== LOCALHOST SIMULATION ===================
    // If we are on localhost, we cannot actually submit to PayU.
    if (window.location.hostname === "localhost") {
      toast.info("Simulating PayU payment locally...");
      // Directly redirect to the success page
      window.location.href = payuSession.surl;
      return;
    }
    ...
    form.method = "POST";
    form.action = payuSession.action;
    form.name = "payuForm";
      ...
      <div className="flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:placeholder-teal-300 pt-20 text-left">
        ...
        <div className="max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]">
          ...
          <div className="p-5">
            ...
            {/* Enroll / Free / My Courses Buttons */}
            <div>
              {isAlreadyEnrolled ? (
